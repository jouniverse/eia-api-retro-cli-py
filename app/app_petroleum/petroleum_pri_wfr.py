...
def petroleumWeeklyHeatingOilAndPropanePrices():
    URL_BASE = "https://api.eia.gov/v2/"

    endpoint = "petroleum/pri/wfr/"
    ...
    # EPD2F = Heating Oil
    # EPLLPA = Propane
    products = ["EPD2F", "EPLLPA"]
    product_labels = ["Heating Oil", "Propane"]
    ...
    #  save df to csv
    df.to_csv(
        f"./output/petroleum_weekly_heating_oil_and_propane_prices_{end_time.strftime('%Y-%m-%d')}.csv",
        index=False,
    )
    ...
    print(
        f"Current ({end_time.strftime('%Y-%m-%d')}) weekly heating oil and propane prices (USD/gal)"
    )
    table_data = [
